...
TOAST UI 제품들은 패키지 매니저를 이용할 수도 있고, 직접 소스 코드를 다운로드할 수도 있다. 하지만 패키지 매니저를 사용하는 것을 권장한다.

### Package Manager 이용

각 패키지 매니저가 제공하는 CLI 도구를 사용하면 쉽게 패키지를 설치할 수 있다. `npm` 을 사용하기 전에 [Node.js](https://nodejs.org/en/)를 미리 설치해야 한다.
...
TOAST UI Grid가 생성될 컨테이너 요소를 만든다.
...
생성자 함수를 이용해 그리드 인스턴스를 생성한다. 그리드의 생성자 함수를 사용하기 전에 각 환경에 맞는 방법을 사용해 그리드 모듈을 불러온다.
...
그리드 생성자는 옵션 객체를 인자로 받는데 `el` 과 `columns` 옵션은 필수로 입력되어야 한다. `el` 옵션은 그리드가 생성될 컨테이너 HTML 엘리먼트를 전달한다. 그리고 `columns` 은 그리드에서 사용되는 데이터의 이름, 헤더, 에디터 등의 컬럼 정보 배열이다. 컨테이너 엘리먼트는 그리드 내부에서 자동으로 생성해주지 않기 때문에 인스턴스를 생성하기 전에 미리 만들어 줘야 한다.
...
그리드에 데이터를 추가하기 전에 데이터의 스키마에 해당하는 칼럼 모델을 정의해야 한다. 컬럼 모델은 `setColumns()` 메서드를 이용해 정의한다.
...
`editor` 속성은 컬럼의 인풋 타입을 결정한다. 예를 들어 `text` 타입은 `input[type=text]` 엘리먼트를 사용해 셀의 값을 설정한다. 이 밖에도 `select`, `checkbox` 을 `editor.type` 으로 사용할 수 있다. `editor.options`에 사용할 수 있는 여러 옵션들은 [셀 에디터 인터페이스 문서](./custom-editor.md)에서 확인할 수 있다.

`setColumns()` 메서드를 이용하지 않고 그리드 인스턴스를 만들면서 `columns` 옵션을 전달할 수 있다.
...
컬럼 모델을 정의했다면 비로소 그리드에 데이터를 입력할 수 있다. 두 가지 방법이 있는데 인스턴스 생성 시 `data` 옵션을 사용하는 방법과 `resetData()` 메서드를 이용하는 방법이다.
