...
새로운 디자인에 맞게 더 세부적으로 스타일을 설정할 수 있도록 테마 API의 옵션이 강화되었으며, 트리 형태로 데이터를 표현할 수 있는 기능도 추가되었습니다.
...
특별한 작업은 필요하지 않습니다. 그리드의 자바스크립트 및 CSS 파일만 변경하면 됩니다.
...
변경된 디자인에 따라 행 높이(`height`)와 관련된 **옵션의 기본 값이 변경**되었습니다.
...
기존 그리드의 레이아웃이 **폐쇄형**이었다면, 3.0 버전에서는 **개방형** 레이아웃으로 변경되었습니다.

이에 따라 테마 API 호출 시 사용하는 옵션 중 불필요한 옵션이 제거되고, 더 세밀하게 구성 요소의 스타일을 조정할 수 있는 옵션이 추가되었습니다.
...
* `grid` : 레이아웃 분리로 공통된 보더와 영역이 사라져 디프리케이트 되었습니다.
...
* `scrollbar.emptySpace `: 실제 스크롤이 생성되는 영역 외 나머지 영역에 대한 배경색을 변경합니다.
...
* `cell.rowHead` : 메타 컬럼 영역의 셀 스타일을 지정합니다.
* `cell.selectedRowHead` : 현재 선택된 메타 컬럼 영역의 셀 스타일을 지정합니다.
* `cell.summary` : 서머리 영역의 셀 스타일을 지정합니다.
...
* 트리 컬럼 제공 : 특정 컬럼의 데이터를 트리 형태로 표현해서 보여줍니다.
* 트리 요소 및 기능
    * 뎁스 : 부모 - 자식 행의 계층적 관계를 표현합니다. 최소 뎁스는 `1`부터 시작합니다.
    * 접기/펴기 버튼 : 자식 행을 가지는 부모 행에 추가되며, 자식 행을 보여주거나 숨김 처리 할 수 있습니다.
    * 폴더/파일 아이콘 : 자식 행 유무 상태를 보여주는 아이콘을 추가할 수 있습니다.
...
* `_extraData.treeState` 프로퍼티로 자식 행에 대한 접기/펴기 초기 상태를 설정할 수 있습니다. 설정되어 있지 않을 경우 기본 값은 닫힘 상태(`COLLAPSE`) 입니다.
...
| 옵션명 | 설명 |
| --- | --- |
| `name` | 트리 데이터를 보여줄 컬럼명 지정 |
| `useIcon` | 폴더/파일 아이콘 사용 유무 설정 |
| `useCascadingCheckbox` | 메타 컬럼 영역의 체크박스 선택 시 부모-자식 관계를 유지한 상태로 선택할지 여부 설정 |
...
| 옵션명 | 설명 |
| --- | --- |
| `parentRowKey` | 추가할 행에 대한 부모 행 키 값 |
